...
def floating_to_binary(floating, line_no):
    if floating < 0:
        raise Exception(f"Negative numbers are not supported, Line{line_no}")
    ...
    if floating != 0:
        ...
        normalized = abs(floating)
        while normalized >= 2:
            normalized /= 2
            exponent += 1

        # Adjust the exponent for the bias
        exponent += 3

        # Extract the mantissa
        mantissa = int((normalized - 1) * (2 ** 5))

    # Convert the components to binary strings
    sign_str = f"{sign:01b}"
    exponent_str = f"{exponent:03b}"
    mantissa_str = f"{mantissa:05b}"

    # Concatenate the binary components
    binary = sign_str + exponent_str + mantissa_str
...
def generate_binary(assembly):
    ...
    for line_no, line in assembly:
        ...
        if instruction in [
            ...
            imm_str = syntax[1][1:]
            binary += registers[register]
            binary += floating_to_binary(float(imm_str), line_no)
        else:
            break
        binary += "\n"
        bin_count += 1
