ISA = {'add' : '0000000', 'sub' : '0000100', 'mov' : ['000100','0001100000'], 'ld' :'001000' , 'st' : '001010', 'mul' : '0011000', 'div' : '0011100000', 'ls' : '010010', 'rs' : '010000', 'or' : '0101100', 'xor' : '0101000', 'and' : '0110000', 'not' : '0110100000', 'cmp' : '0111000000', 'jmp' : '011110000', 'jgt' : '111010000', 'jlt' : '111000000', 'je' : '111110000','hlt':'1101000000000000'}
...
def scan_input():  # Take input from termimal
    line_no = 0
...
def update_var_label(raw_assembly):
    ...
    for line_no, line in raw_assembly:
        ...
        if(line_list[0] == 'var'):
            if var_ntbegin:
                raise Exception(f'Variables not declared at beginning, line {line_no}')
            if( 2 != len(line_list)):
                raise Exception(f'General Syntax Error, line {line_no}')
            else:
...
def verify_regregreg(syntax,line_no):
    ...
    for register in syntax:
        if register == 'FLAGS':
            raise Exception(f'Illegal use of FLAGS register, line {line_no}')
        if register not in registers:
            raise Exception(f'Invalid Register name, line {line_no}')
    return None
...
def verify_jump(syntax,line_no):
    ...
    if ((label in variables )and(label not in labels )):
        raise Exception(f'Misuse of labels as variables, line {line_no}')
    if ((label not in labels )):
        raise Exception(f'Use of undefined labels, line {line_no}')
    return None
...
def verify_regvar(syntax,line_no):
    ...
    if ((variable not in variables )and( variable  in labels )):
        raise Exception(f'Misuse of variables as label, line {line_no}')
    if ((variable not in variables )):
        raise Exception(f'Use of undefined variables, line {line_no}')
    return None
...
def verify_regimm(syntax,line_no):
    ...
    for x in range(1, len(imm)):
        if not imm[x].isdigit():
            raise Exception(f'General Syntax Error, line {line_no}')
    if int(imm[1:])>127:
        raise Exception(f'Illegal Immediate values, line {line_no}')
...
def inspect_syntax(assembly):
    if assembly[-1][1] != ['hlt']:
        line_no = assembly[-1]
        raise Exception(f'Missing Halt instruction at end, line {line_no}')
    for line_no,line in assembly : 
        instruction = line[0]
        if(instruction not in ISA):
            raise Exception(f'Invalid Instruction, line {line_no}')
        syntax = line[1:]
...
def main():
    ...
    if (len(raw_assembly)>128):
        raise Exception(f'Assembler can write upto 128 lines')
    assembly = update_var_label(raw_assembly)
    for label in labels :
        labels[label]  = dec2bin(labels[label]-len(variables)-1)
